...
void startWindow(struct Window *window)
{
    ...
    make_VUIE_Window(window, "Test Window");
...
void containerInit()
{
    ...
    // EXAMPLEBOX has a position conection to MOUSE (Trigger it On and Off with "transform.pos.ReferToParent")
    ...
    // C2 is INSIDE exampleBox
    exampleBox.addChild(&exampleBox, c2.getSelf(&c2));
    // C2 Position DOES NOT REFER to EXAMPLEBOX position
...
// DISPLAY function, Occours after the main window's CHILDS, so it render on top of it
void display(struct Window *window, void *args)
{
    // Controls for moving EXAMPLEBOX
    mouseControls();

    // Controls for changing the Window Scale, resulting in changing everything inside of it
    keyControls();
    ...
    Draw_Pixels(10, points, RGBColor(0.7, 0.7, 0.7), 1, window->GlobalScale);

    Draw_pixel(mouse.pos.x, mouse.pos.y, RGBColor(1.0, 1.0, 1.0), 1, window->GlobalScale);
}
