...
void Draw_pixel(int x, int y, Color *color, int w, int scale) // draw a pixel at x/y with rgb
{
...
void Draw_Pixels(int amount, Vector2 *points, Color *color, int w, int scale)
{
    ...
    {
        glVertex2i(points[i].x * scale, points[i].y * scale);
    }
    glEnd();
}

void Draw_line(int x1, int y1, int x2, int y2, Color *color, int w, int scale)
{
    ...
    glLineWidth(scale * w);
    glBegin(GL_LINES);
    glVertex2i(x1 * scale, y1 * scale);
    glVertex2i(x2 * scale, y2 * scale);
    glEnd();
}

void Draw_quads(int x1, int y1, int x2, int y2, Color *color, int scale)
{
    setGLColor(color);

    glBegin(GL_QUADS);
    glVertex2i(x1 * scale, y1 * scale);
    glVertex2i(x1 * scale, y2 * scale);
    glVertex2i(x2 * scale, y2 * scale);
    glVertex2i(x2 * scale, y1 * scale);
    glEnd();
}
...
void Draw_PolygonOutline(Vector2 *points, int amount, Color *color, int width, int scale)
{
    if (width < 1)
    {
        return;
